...
# Class crawl data
class lt_laptop888(scrapy.Spider):
    name = 'lt_laptop888'
    start_urls = ['https://laptop88.vn/laptop-moi.html?page=1']

    def parse(self, response):
        for product in response.css('#product > div.p-item'):
            item_link = 'https://laptop88.vn' + product.css('div.p-emtry > a::attr(href)').get()

            if item_link:
                temp_ten = product.css('div.p-emtry > a::text').get()[18:]
                ten0 = temp_ten.split(' ')[0:5]
                ten1 = ' '.join(ten0).title()
                ten = name_processing(ten1)

                thuonghieu = str(item_link.split('laptop-')[1].split('-')[0])
                if thuonghieu == None:
                    thuonghieu = ''
                ...
                item = {
                    'ten': ten,
                    'url': item_link,
                    'image': 'https://laptop88.vn' + str(product.css('div.p-img > a > img::attr(src)').get()),
            ...
            if next_page_number <= 6:
                next_page_url = 'https://laptop88.vn/laptop-moi.html?page={}'.format(next_page_number)
    ...
    def get_detail(self, response):
        ...
        option_old_price = format_price(response.css('span.unprice > span::text').get())
        option_new_price = format_price(response.css('span.price::text').get())

        tskt = response.css('div.bg-white.parameter').get()
        mota = response.css('div.bg-white').get()
