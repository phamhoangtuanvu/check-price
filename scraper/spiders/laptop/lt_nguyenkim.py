...
def name_processing(name):
    unprocess_name = str(name)[0:35].title()
    processed_name = []
...
class lt_nguyenkim(scrapy.Spider):
    name = 'lt_nguyenkim'
    start_urls = ['https://www.nguyenkim.com/may-tinh-xach-tay/']
    ...
    def get_detail(self, response):
        ...
        option_old_price = format_price(response.css('div.price_promotion > div > span::text').get())
        option_new_price = format_price(response.css('div.product_info_price_value-final > span::text').get())

        tskt = response.css('.productSpecification_brief table').get()
        mota = response.css('#content_description').get().replace('display: none;', '').replace('src', 'd-src').replace('data-src', 'src')
