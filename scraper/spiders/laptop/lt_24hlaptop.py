...
# Class crawl data
class lt_24laptop(scrapy.Spider):
    name = 'lt_24laptop'
    start_urls = ['http://24laptop.vn/menu-product-1-147-1_laptop-dell.html']

    def parse(self, response):
        for product in response.css('ul.sanpham > li'):
            item_link = 'http://24laptop.vn/' + product.css('div.boxpro > a::attr(href)').get()

            if item_link:
                temp_ten = product.css('div.boxpro > a::text').get()
                ten0 = temp_ten.split(' ')[1:4]
                ten = ' '.join(ten0)
            ...
            next_page_url = 'http://24laptop.vn/' + str(response.css('#content > div:nth-child(5) > a:nth-child(7)::attr(href)').extract_first())
    ...
    def get_detail(self, response):
        ...
        option_new_price = format_price(response.css('div.chitietsp > div:nth-child(2)::text').get())

        tskt = response.css('#content > div.detail_pro > div:nth-child(4) > div:nth-child(27)').get()
        mota = response.css('#content > div.detail_pro > div:nth-child(4) > div:nth-child(16)').get()
        ...
        item['image'] = 'http://24laptop.vn/' + str(response.css('div.detail_proimg > img::attr(src)').get())
        item['tskt'] = tskt
        item['mota'] = mota
        return item