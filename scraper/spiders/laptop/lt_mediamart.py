...
# Class crawl data
class lt_mediamart(scrapy.Spider):
    name = 'lt_mediamart'
    start_urls = ['https://mediamart.vn/laptop?trang=1']

    def parse(self, response):
        for product in response.css('.pl18-item-ul li'):
            item_link = 'https://mediamart.vn' + product.css('.pl18-item-image a::attr(href)').get()
            if item_link:
                ten1 = product.css('.pl18-item-name a::attr(title)').get()
                ten = name_processing(ten1)

                thuonghieu = str(item_link.split('/')[5].split('-')[1])
            ...
            next_page_number = int(response.url.split('trang=')[1]) + 1
            if next_page_number <= 5:
                next_page_url = 'https://mediamart.vn/laptop?&trang={}'.format(next_page_number)
