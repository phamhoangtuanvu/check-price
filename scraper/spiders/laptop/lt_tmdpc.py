...
# Class crawl data
class lt_tmdpc(scrapy.Spider):
    name = 'lt_tmdpc'
    start_urls = ['https://tmdpc.vn/laptop-theo-hang.html?page=1']

    def parse(self, response):
        for product in response.css(' div.__list-product > div.product-ct'):
            item_link = 'https://tmdpc.vn/' + product.css('div.wrap-content > div::attr(onclick)').get()[16:].replace("'","")

            if item_link:
                temp_ten = product.css('div.__product-name::text').get().split(' ')[0:6]
                ten1 = ' '.join(temp_ten).title()
                ten = name_processing(ten1)

                thuonghieu = str(item_link.split('/')[3].split('-')[1])
                if thuonghieu == 'macbook' or thuonghieu == 'imac':
                    thuonghieu = 'apple'
                ...
                item = {
                    'ten': ten,
                    'url': item_link,
                    'image': 'https://tmdpc.vn' + str(product.css('div.__wrap-img > img::attr(data-original)').get()),
                    'ngay': date.today().strftime("%Y-%m-%d"),
                    'loaisanpham': 'laptop',
                    'thuonhhieu': thuonghieu,
                }
                yield scrapy.Request(url=item_link, meta={'item': item}, callback=self.get_detail)

            next_page_url = 'https://tmdpc.vn' + str(response.css('div.paging > a:nth-child(8)::attr(href)').extract_first())
    ...
    def get_detail(self, response):
        ...
        option_old_price = format_price(response.css('div.old_price_detail > span::text').get())
        option_new_price = format_price(response.css('#product-price-holder > span::text').get())

        tskt = response.css('div.product-spec').get()
