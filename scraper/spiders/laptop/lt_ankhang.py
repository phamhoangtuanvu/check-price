...
# Class crawl data
class lt_ankhang(scrapy.Spider):
    name = 'lt_ankhang'
    start_urls = ['https://www.ankhang.vn/laptop.html']

    def parse(self, response):
        for product in response.css('div.product_list_box > ul > li.p-item-2021'):
            item_link = 'https://www.ankhang.vn' + product.css('div.p-container > a.p-name::attr(href)').get()
            if item_link:
                ten1 = product.css('div.p-container > a.p-name::text').get()
            ...
            next_page_url = 'https://www.ankhang.vn' + str(response.css('div.product_list_box > div > a:nth-child(8)::attr(href)').extract_first())
    ...
    def get_detail(self, response):
        ...
        option_old_price = format_price(response.css('span.pro-oldprice::text').get())
        option_new_price = format_price(response.css('span.pro-price::text').get())

        tskt = response.css('div.content-item.paramater.position-relative').get()
        mota = response.css('#tab2').get()
        ...
        item['image'] = response.css('#Zoomer > img::attr(src)').get()
        item['tskt'] = tskt
        item['mota'] = mota
        return item