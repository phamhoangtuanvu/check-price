...
def get_attr_from_name(name):
    ...
    list_attr_mausac = ['Xám', 'Đỏ', 'Đen', 'Lục', 'Lam', 'Đồng', 'Vàng', 'Xanh', 'Đen', 'Trắng', 'Thạch', 'Anh', 'lá',
                        'ngọc', 'lam', 'Sapphire']
...
#class craw
class lt_hnammobile(scrapy.Spider):
    name = 'lt_hnammobile'
    start_urls = ['https://www.hnammobile.com/laptop?filter=p-desc']

    def parse(self, response):
        for product in response.css('.list-products div.product-item-list'):
            item_link = product.css('.product-image a::attr(href)').get()

            thuonghieu = str(item_link.split('/')[4].split('-')[0])
            if thuonghieu == 'macbook' or thuonghieu == 'imac':
                thuonghieu = 'apple'


            ten1 = product.css('.product-name a::text').get()
            temp_ten = ten1.split(' ')[0:3]
            ten = ' '.join(temp_ten).title()
            ...
            yield scrapy.Request(url=item_link, meta={'item': item, 'price': price},
    ...
    def get_detail(self, response):
        ...
        item['mota'] = response.css('.article-news .article-main-content').get().replace('display: none;', '').replace(
            'src', 'd-src').replace('data-src', 'src')
        return item