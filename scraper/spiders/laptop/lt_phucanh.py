...
# Lớp crawl dữ liệu
class lt_phucanh(scrapy.Spider):
    name = 'lt_phucanh'
    ...
    def parse(self, response):
        ...
        for product in response.css('#content-left .category-pro-list ul.product-list li'):
            item_link = 'https://www.phucanh.vn' + product.css('div.p-container > a::attr(href)').get()

            ten1 = product.css('div.p-container > h3::text').get()
            temp_ten = ten1.split(' ')[0:6]
            ten2 = ' '.join(temp_ten).title()
            ten = name_processing(ten2)
